...
class Watchlist {
    ...
    @Autowired
    var lightStreamerClient: EtoroLightStreamerClient

    val SAVED_LIST_PATH = "watchlist.json"
	
    constructor(lightStreamerClient: EtoroLightStreamerClient){
        this.lightStreamerClient = lightStreamerClient
        this.lightStreamerClient.watchlist = this
    }
    ...
    fun getPrice(id: String, type: EtoroPositionTypeEx, discounted: Boolean): Double {
        val asset = watchlist[id]
        if (asset != null) {
            if (type == EtoroPositionTypeEx.BUY && asset.buy != null) {
                if (discounted) return asset.askDiscounted
                return asset.buy!!
            } else if (type == EtoroPositionTypeEx.SELL && asset.sell != null) {
